...
describe('composeSetupUri', () => {
  test('get Setup Uri for category, password', () => {
    expect.assertions(1);
    expect(composeSetupUri(CATEGORIES.switch, '84131633', '')).toBe('X-HM://0081YCYEP');
  });

  test('get Setup Uri for category, password and setupId', () => {
    expect.assertions(1);
    expect(composeSetupUri(CATEGORIES.switch, '84131633', '3QYT')).toBe('X-HM://0081YCYEP3QYT');
  });
});
...
describe('composeQrCode', () => {
  test('get Setup Uri for switch', async () => {
    expect.assertions(1);
    const setupUri = composeSetupUri(CATEGORIES.switch, '84131633', '3QYT');
