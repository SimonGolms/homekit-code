...
export const composeQrCode = async ({ pairingCode, setupUri }: ComposeQrCode) => {
  const qrCode = await toString(setupUri, QR_CODE_STRING_OPTIONS);
...
<svg viewBox="0 0 400 540" xmlns="http://www.w3.org/2000/svg">
  ...
  <defs>
    ...
    <symbol id="qrCode" ${qrCode.slice(5, -7)}</symbol>
  </defs>
